let WALLET={'net':'ropsten','type':'http','main':{'http':'https://mainnet.infura.io','wss':'wss://mainnet.infura.io/ws','api':'https://api.etherscan.io','href':'https://etherscan.io'},'ropsten':{'http':'https://ropsten.infura.io','wss':'wss://ropsten.infura.io/ws','api':'https://api-ropsten.etherscan.io','href':'https://ropsten.etherscan.io'},'tokens':['empty for eth','nitrotoken'],'tokenABI':[{"constant":!0,"inputs":[],"name":"name","outputs":[{"name":"","type":"string"}],"payable":!1,"type":"function"},{"constant":!0,"inputs":[],"name":"decimals","outputs":[{"name":"","type":"uint8"}],"payable":!1,"type":"function"},{"constant":!0,"inputs":[{"name":"_owner","type":"address"}],"name":"balanceOf","outputs":[{"name":"balance","type":"uint256"}],"payable":!1,"type":"function"},{"constant":!0,"inputs":[],"name":"symbol","outputs":[{"name":"","type":"string"}],"payable":!1,"type":"function"}],'nitrotoken':{'address':'0xdF60Ce3dE7b799Ba695A3F049E03c94f8b70fD6A','contract':null}};let storage=new function(){this.wallet='',this.address='',this.tx='',this.time=0,this.MAIN=null,this.load=function(){if(!storage.hasData())
return;let data=JSON.parse(localStorage[WALLET.net]);storage.wallet=data.wallet;storage.address=data.address;storage.time=data.time},this.save=function(){localStorage[WALLET.net]=storage.wallet!=''?JSON.stringify({'wallet':storage.wallet,'address':storage.address,'tx':storage.tx,'time':storage.time}):''},this.hasData=function(){return(typeof localStorage[WALLET.net]!=='undefined'&&localStorage[WALLET.net]!='')},this.hasStorage=function(){return(typeof(Storage)!=="undefined")},this.remove=function(){storage.wallet='';storage.address='';storage.tx='';localStorage.removeItem(WALLET.net)},this.reset=function(){storage.address='';storage.time=0}}
let wallet=new function(){this.web3=null,this.stateBackup=-1,this.timer=1800000,this.coins=[{'icon':'<span class="ethereum"></span>','name':' Eth','balance':-2},{'icon':'<span class="ethereum"></span>','name':' Nitro','balance':-2}],this.state=function(){if(storage.hasStorage()&&storage.hasData()&&storage.wallet!=''){if(storage.address!=='')
...
wallet.MAIN=mainUpdate;if(WALLET.type=="http"){wallet.web3=new Web3(new Web3.providers.HttpProvider(WALLET[WALLET.net][WALLET.type]));setInterval(()=>{wallet.update();wallet.MAIN()},2000)}else{wallet.web3=new Web3(new Web3.providers.WebsocketProvider(WALLET[WALLET.net][WALLET.type]));wallet.web3.eth.subscribe('newBlockHeaders',wallet.update)}
for(let i=1;i<WALLET.tokens.length;i++)
WALLET[WALLET.tokens[i]].contract=new wallet.web3.eth.Contract(WALLET.tokenABI,WALLET[WALLET.tokens[i]].address);console.log("web3 :"+wallet.web3.version)},this.update=function(){wallet.updateTimer(!1);wallet.updateNavAccount();for(let i=0;i<wallet.coins.length;i++){if(wallet.state()==2)
